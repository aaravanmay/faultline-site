...
(function () {
  ...
  function sparkline(vals, color, w, h){ color=color||"#38BDF8"; w=w||62; h=h||26; if(!vals||!vals.length) return ""; if(vals.length<2) vals=[vals[0],vals[0]];
  ...
  <a class="sb-logo" href="/index.html" title="faultline home"><svg width="26" height="18" viewBox="0 0 40 28" fill="none"><path d="M3 10H17" stroke="#E6EBF2" stroke-width="3.2" stroke-linecap="round"/><path d="M17 10L23 18" stroke="#38BDF8" stroke-width="3.2" stroke-linecap="round"/><path d="M23 18H37" stroke="#E6EBF2" stroke-width="3.2" stroke-linecap="round"/></svg><span class="sb-word">fault<b>line</b></span></a>
  <nav class="sb-nav">
...
<header class="topbar">
  ...
  async function injectLayout(){
    ...
    // regressions badge
    if(ctx.project){ getRegressions(ctx.orgId,"open").then(function(rs){ var b=shell.querySelector('#faultsBadge'); if(b){ if(rs&&rs.length){ b.textContent=rs.length; b.classList.remove("hide"); } else b.classList.add("hide"); } }).catch(function(){}); }
  }
